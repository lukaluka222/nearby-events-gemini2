...
本文: ${allText}`;
    ...
    // 2.5) Geminiがゼロ件ならリンクfallbackで暫定イベントを生成
    ...
    // 3) 正規化 → デデュープ → ドメイン上限 → （軽シャッフル）→ 半径フィルタ → スコア
    const norm = events.map(normalize);
    const uniq = dedupeAndCap(norm, 3);
    // 似た候補が多いときのバラけ対策
    ...
    if (!ranked.length) {
      ...
      if (debug) {
        return res.status(200).json({ ok: true, count: 1, errors, sample: one });
      }
